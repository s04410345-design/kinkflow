...
export default function ReportReviewPanel({ onMessage }: { onMessage: (message: string) => void }) {
  ...
  const [decisions, setDecisions] = useState<Record<string, { action: ResolutionAction; note: string }>>({});
  ...
  const resolve = async (report: AdminReport, status: 'resolved' | 'dismissed') => {
    ...
    const result = await resolveAdminReport(report.id, status, status === 'dismissed' ? 'none' : decision.action, decision.note);
