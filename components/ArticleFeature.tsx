...
function AuthorWorkspace({ nodesData, isMember, verification, setVerification }: { nodesData: GraphNode[]; isMember: boolean; verification: AuthorVerification | null; setVerification: (value: AuthorVerification | null) => void }) {
  ...
  const saveDraftSilently = useCallback(async () => {
    ...
    const result = await updateArticleDraft(editingId, title, excerpt, buildCurrentDocument(), selectedNodeIds, undefined, nodesData);
    if (result.ok) {
  ...
  }, [articleDocument, articleSaving, editingId, excerpt, markdown, nodesData, selectedNodeIds, title, videoUrl]);
  ...
  }, [editorOpen, editingId, title, excerpt, markdown, selectedNodeIds, saveDraftSilently]);
  ...
  const resetEditor = () => {
    if (autosaveTimerRef.current) clearTimeout(autosaveTimerRef.current);
    setEditorOpen(false); setEditingId(null); setTitle(''); setExcerpt(''); setMarkdown(''); setArticleDocument(createEmptyArticleDocument()); setVideoUrl(''); setSelectedNodeIds([]); setPreviewOpen(false); setDraftStatus('idle'); setLastSavedAt(null);
  };

  const openNewArticle = () => {
    resetEditor(); setSelectedNodeIds([]); setEditorOpen(true); setNotice(null);
  };

  const openArticle = (article: EditableArticle) => {
    const document = parseArticleDocument(article.body_json, 'draft');
    setEditingId(article.id); setTitle(article.title); setExcerpt(article.excerpt); setMarkdown(document.introMarkdown); setArticleDocument(document); setVideoUrl(document.media.find((media) => media.type === 'video')?.url || ''); setSelectedNodeIds((article.nodeIds || []).slice(0, 3)); setPreviewOpen(false); setDraftStatus('saved'); setLastSavedAt(article.updated_at || null); setEditorOpen(true); setNotice(null);
  };
  ...
  const saveArticle = async () => {
    ...
    const result = editingId ? await updateArticleDraft(editingId, title, excerpt, currentDocument, selectedNodeIds, undefined, nodesData) : await createArticleDraft(title, excerpt, currentDocument, selectedNodeIds, undefined, nodesData);
    if (!result.ok) {
...
export default function ArticleFeature({ nodesData, initialNodeId = null, onBackToNode, isMember = false }: ArticleFeatureProps) {
  ...
  const filterNodes = useMemo(() => getMindmapTopicNodes(nodesData).filter((node) => articles.some((article) => article.nodeIds.includes(node.id))), [articles, nodesData]);
