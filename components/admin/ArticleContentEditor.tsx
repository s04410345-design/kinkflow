...
type MediaTarget = 'cover' | { sectionIndex: number };
  ...
  const uploadImage = async (file: File, target: MediaTarget) => {
    ...
    try {
      ...
      if (target === 'cover') {
        ...
        updateSectionMedia(target.sectionIndex, 0, {});
        setDocument((current) => ({
          ...current,
          sections: current.sections.map((section, index) => index !== target.sectionIndex ? section : { ...section, media: section.media.length ? section.media : [{ ...emptyMedia('image'), url: payload.url || '' }] }),
        }));
      }
          ...
          <div className="mt-4 space-y-4">
            {document.sections.map((section, sectionIndex) => (
              <div key={section.id} className="rounded-2xl border border-[#D1C6B4]/60 bg-white p-4">
                ...
                <div className="mt-3 rounded-xl bg-[#FDFBF7] p-3"><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-sm font-black">本章媒體</p><div className="flex gap-2"><button type="button" onClick={() => addSectionMedia(sectionIndex, 'image')} className="rounded-lg border border-[#D1C6B4]/70 px-3 py-1.5 text-xs font-bold">＋圖片</button><button type="button" onClick={() => addSectionMedia(sectionIndex, 'video')} className="rounded-lg border border-[#D1C6B4]/70 px-3 py-1.5 text-xs font-bold">＋影片</button></div></div>{section.media.map((media, mediaIndex) => <div key={`${section.id}-media-${mediaIndex}`} className="mt-3 grid gap-2 rounded-xl border border-[#D1C6B4]/50 bg-white p-3 md:grid-cols-[100px_1fr_auto]"><select value={media.type} onChange={(event) => updateSectionMedia(sectionIndex, mediaIndex, { type: event.target.value === 'video' ? 'video' : 'image' })} className="rounded-lg border border-[#D1C6B4]/70 p-2 text-xs"><option value="image">圖片</option><option value="video">影片</option></select><div className="space-y-2"><input value={media.url} onChange={(event) => updateSectionMedia(sectionIndex, mediaIndex, { url: event.target.value })} className="w-full rounded-lg border border-[#D1C6B4]/70 p-2 text-sm" placeholder="https://..." /><input value={media.alt} onChange={(event) => updateSectionMedia(sectionIndex, mediaIndex, { alt: event.target.value })} className="w-full rounded-lg border border-[#D1C6B4]/70 p-2 text-sm" placeholder="替代文字／影片描述" /><input value={media.caption} onChange={(event) => updateSectionMedia(sectionIndex, mediaIndex, { caption: event.target.value })} className="w-full rounded-lg border border-[#D1C6B4]/70 p-2 text-sm" placeholder="媒體說明（選填）" /></div><button type="button" onClick={() => removeSectionMedia(sectionIndex, mediaIndex)} className="self-start text-xs font-bold text-[#9F1239]">移除</button></div>)}</div>
              </div>
            ))}
          </div>
