...
function AuthorWorkspace({ nodesData, isMember, verification, setVerification }: { nodesData: GraphNode[]; isMember: boolean; verification: AuthorVerification | null; setVerification: (value: AuthorVerification | null) => void }) {
  ...
  const openNewArticle = () => {
    resetEditor();
    setSelectedNodeId(nodesData.find((node) => node.level > 0)?.id || '');
    setEditorOpen(true);
    setNotice(null);
  };
  ...
  const openArticle = (article: EditableArticle) => {
    ...
    setMarkdown(typeof article.body_json?.markdown === 'string' ? article.body_json.markdown : '');
    setSelectedNodeId(article.nodeIds?.[0] || '');
    setPreviewOpen(false);
    setDraftStatus('saved');
    setLastSavedAt(article.updated_at || null);
    setEditorOpen(true);
    setNotice(null);
  };
  ...
  const saveArticle = async () => {
    ...
    const result = editingId
      ? await updateArticleDraft(editingId, title, excerpt, markdown, selectedNodeId ? [selectedNodeId] : [])
      : await createArticleDraft(title, excerpt, markdown, selectedNodeId ? [selectedNodeId] : []);
    if (!result.ok) {
      setDraftStatus('error');
      setNotice(result.message || '文章儲存失敗。');
    } else {
      setDraftStatus('saved');
      setLastSavedAt(new Date().toISOString());
      setEditingId(result.articleId || editingId);
      setEditorOpen(false);
      setMyArticles(await fetchMyArticles());
      setNotice(editingId ? '文章草稿已更新。' : '文章草稿已建立。');
    }
  ...
  const draftStatusLabel = draftStatus === 'saving'
    ...
    : draftStatus === 'error'
      ? '自動儲存失敗'
      : lastSavedAt
        ? `已儲存 ${formatArticleDate(lastSavedAt) || '剛剛'}`
        : '尚未儲存';

  return (
    <div className="mt-6 rounded-3xl border border-[#D1C6B4]/50 bg-white/80 p-5 shadow-sm md:p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div><p className="text-xs font-black uppercase tracking-[0.18em] text-[#A27B21]">Author Studio</p><h2 className="mt-1 text-xl font-black">認證作者工作區</h2><p className="mt-2 text-sm leading-6 text-[#4A4238]/70">專題誌適合長篇文章。可以用 Markdown 排版，圖片使用 <code>![圖片](網址)</code>，影片可以貼成 Markdown 連結。</p></div>
        {verification?.status === 'approved' && <button type="button" onClick={openNewArticle} className="rounded-xl bg-[#1A1612] px-4 py-2 text-sm font-bold text-white">新增長文</button>}
      </div>
      ...
      {!verification || verification.status === 'none' ? <div className="mt-5"><p className="text-sm leading-6 text-[#4A4238]/75">申請通過後，你可以建立、編輯並發布自己的專題文章。</p><textarea value={applicationText} onChange={(e) => setApplicationText(e.target.value)} maxLength={5000} placeholder="請介紹你想寫的主題、內容方向與社群經驗（至少 30 字）" className="mt-3 min-h-28 w-full rounded-xl border border-[#D1C6B4]/60 bg-white p-3 text-sm outline-none focus:border-[#A27B21]" /><button type="button" onClick={() => void submitApplication()} disabled={applicationSaving} className="mt-3 rounded-xl bg-[#A27B21] px-4 py-2 text-sm font-bold text-white disabled:opacity-50">{applicationSaving ? '送出中…' : '申請認證作者'}</button></div>
        : verification.status === 'pending' ? <p className="mt-5 rounded-xl bg-[#FFF9E8] p-4 text-sm text-[#6B5310]">申請審核中。管理員確認後，就能開啟長文編輯器。</p>
          : verification.status === 'rejected' ? <div className="mt-5"><p className="text-sm text-[#9F1239]">這次申請尚未通過{verification.review_note ? `：${verification.review_note}` : '，你可以補充內容後重新申請。'}</p><textarea value={applicationText} onChange={(e) => setApplicationText(e.target.value)} maxLength={5000} className="mt-3 min-h-28 w-full rounded-xl border border-[#D1C6B4]/60 bg-white p-3 text-sm" /><button type="button" onClick={() => void submitApplication()} disabled={applicationSaving} className="mt-3 rounded-xl bg-[#A27B21] px-4 py-2 text-sm font-bold text-white">重新申請</button></div>
            : <div className="mt-5">
              <p className="mb-3 rounded-xl bg-[#EEF4EA] p-4 text-sm text-[#47633C]">已通過認證。你的文章會先以草稿保存，再由你確認後發布。</p>
              {editorOpen && <div className="mb-4 rounded-2xl border border-[#D1C6B4]/60 bg-[#FDFBF7] p-4">
                <div className="flex flex-wrap items-center justify-between gap-3"><h3 className="font-black">{editingId ? '編輯文章' : '新增長文'}</h3><div className="flex items-center gap-3"><span aria-live="polite" className={`text-xs font-bold ${draftStatus === 'error' ? 'text-[#B91C1C]' : 'text-[#64748B]'}`}>{draftStatusLabel}</span><button type="button" onClick={resetEditor} className="text-sm font-bold text-[#4A4238]/60">關閉</button></div></div>
                <div className={previewOpen ? 'mt-4 grid gap-4 lg:grid-cols-2' : 'mt-4'}>
                  <div>
                    <input value={title} onChange={(e) => setTitle(e.target.value)} maxLength={180} placeholder="文章標題" className="w-full rounded-xl border border-[#D1C6B4]/60 bg-white p-3 text-sm" />
                    <input value={excerpt} onChange={(e) => setExcerpt(e.target.value)} maxLength={500} placeholder="文章摘要（最多 500 字）" className="mt-3 w-full rounded-xl border border-[#D1C6B4]/60 bg-white p-3 text-sm" />
                    <select value={selectedNodeId} onChange={(e) => setSelectedNodeId(e.target.value)} className="mt-3 w-full rounded-xl border border-[#D1C6B4]/60 bg-white p-3 text-sm"><option value="">不指定 Mind Map 節點</option>{nodesData.filter((node) => node.level > 0).map((node) => <option key={node.id} value={node.id}>{node.label}</option>)}</select>
                    <textarea value={markdown} onChange={(e) => setMarkdown(e.target.value)} placeholder={'使用 Markdown 編輯正文\n\n![圖片](https://...)\n\n[觀看影片](https://...)'} className="mt-3 min-h-72 w-full rounded-xl border border-[#D1C6B4]/60 bg-white p-3 font-mono text-sm leading-6" />
                  </div>
                  {previewOpen && <div className="min-h-72 rounded-xl border border-[#D1C6B4]/60 bg-white p-4"><p className="mb-3 text-xs font-black uppercase tracking-[0.16em] text-[#A27B21]">Preview / 預覽</p><MarkdownPreview markdown={markdown} /></div>}
                </div>
                <div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => setPreviewOpen((value) => !value)} className="rounded-xl border border-[#A27B21]/60 bg-[#FFF9E8] px-4 py-2 text-sm font-bold text-[#6B5310]">{previewOpen ? '關閉預覽' : '開啟預覽'}</button><button type="button" onClick={() => void saveArticle()} disabled={articleSaving} className="rounded-xl bg-[#1A1612] px-4 py-2 text-sm font-bold text-white">{articleSaving ? '儲存中…' : '儲存草稿'}</button><button type="button" onClick={resetEditor} className="rounded-xl border border-[#D1C6B4]/70 px-4 py-2 text-sm font-bold">取消</button></div>
                <p className="mt-2 text-xs leading-5 text-[#64748B]">編輯既有草稿時，停止輸入約 1.5 秒會自動儲存；新文章第一次仍請按「儲存草稿」建立文章。</p>
              </div>}
              <div className="space-y-2">{myArticles.map((article) => <div key={article.id} className="flex flex-col gap-3 rounded-xl border border-[#D1C6B4]/50 bg-white p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-bold">{article.title || '未命名文章'}</p><p className="mt-1 text-xs text-[#4A4238]/60">{article.status === 'published' ? '已發布' : '草稿'} · 更新於 {formatArticleDate(article.updated_at) || '日期未提供'}</p></div><div className="flex gap-2"><button type="button" onClick={() => openArticle(article)} className="rounded-lg border border-[#D1C6B4]/70 px-3 py-1.5 text-xs font-bold">編輯</button>{article.status === 'draft' && <button type="button" onClick={() => void publish(article.id)} disabled={articleSaving} className="rounded-lg bg-[#A27B21] px-3 py-1.5 text-xs font-bold text-white">發布</button>}</div></div>)}{myArticles.length === 0 && <p className="rounded-xl border border-dashed border-[#D1C6B4] p-4 text-sm text-[#4A4238]/60">還沒有自己的文章，按右上角「新增長文」開始。</p>}</div>
            </div>}
    </div>
  );
}
